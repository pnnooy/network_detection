...
| 阶段 | 内容 | 建议时间 | Commit message 示例 |
|---|---|---|---|
| **Phase 1** | 创建个人分支；搭建模块骨架；**李哲交付 `mock_data/mock_packets.json`**；其余人基于统一格式确认接口无误 | 第1周前半 | `[capture] feat: 抓包框架骨架 + mock数据交付` |
| **Phase 2** | 完成核心检测逻辑；基于 mock 数据跑通基础用例（如成功检出SQL注入/暴力破解/端口扫描）| 第1周后半-第2周 | `[bruteforce] feat: 核心检测逻辑实现完成` |
| **Phase 2.5** | **接口对齐检查**：每人用真实输出跑一次 `aggregator.py` 骨架，确认告警格式与接口规范完全一致 | 第2周末 | — |
| **Phase 3** | 编写 `tests/test_xxx.py` 单元测试，覆盖正常/异常场景；补充日志与异常处理 | 第3周前半 | `[anomaly] test: 单元测试与异常处理完成` |
| **Phase 4** | 补充模块内文档/注释/使用示例；**真实抓包模块替换mock数据，与其余模块联调**；提交 PR 到 main | 第3周后半 | `[gui] feat: 文档补充与全链路联调完成` |
| **Phase 5** | 根据 Review 意见修改；准备演示数据/截图/答辩素材 | 第4周 | `[signature] docs: 最终版本，验收就绪` |
...
| 周次 | 里程碑 |
|---|---|
| 第1周 | 抓包模块骨架 + mock数据交付；其余模块基于mock数据开发骨架 |
| 第2周 | 各检测模块核心功能实现完成（基于mock数据可正确检出对应攻击）；**周末接口对齐检查** |
| 第3周 | 单元测试通过；真实抓包模块与检测模块联调；PR提交 |
| 第4周 | GUI汇总展示完成；最终验收；答辩材料（PPT/演示视频/测试数据）准备完毕 |
