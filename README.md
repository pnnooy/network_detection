...
| **Phase 2** | 完成核心检测逻辑；基于 mock 数据跑通基础用例（如正确检出SQL注入攻击行为/暴力破解行为/端口扫描行为）| 7/14 ~ 7/17 | `[bruteforce] feat: 核心检测逻辑实现完成` |
| **Phase 2.5** | **接口对齐检查**：每人用真实输出跑 `aggregator.py`，确认告警格式含 `behavior_id`、`description` 为行为导向语态 | 7/18(五) | — |
| **Phase 3** | 编写单元测试，覆盖正常/异常场景；补充日志与异常处理 | 7/19 ~ 7/21 | `[anomaly] test: 单元测试与异常处理完成` |
| **Phase 4** | 真实抓包替换mock数据联调；补充文档/注释；提 PR 合入 main | 7/22 ~ 7/24 | `[gui] feat: 全链路联调 + PR合入` |
| **Phase 5** | PPT、演示数据、截图、结题报告 | 7/25 ~ 8/1 | `[signature] docs: 答辩材料准备完成` |
| **🏁 提前提交** | 源码 + 结题报告打包提交 | **8/2(日)** | — |
| **缓冲** | 应对意外问题 | 8/3 ~ 8/4 | — |
| **📅 官方截止** | 结题报告提交截止 | 8/5(三) | — |

> **Phase 2.5 提前到周五**：7/18(五) 由韩宇飞运行 `aggregator.py` 读取各模块实际输出，校验告警格式。发现问题周末修正，不挤占联调时间。
...
5. **Phase 2.5**（7/18 周五），韩宇飞运行 `aggregator.py` 对各模块实际输出做接口格式校验，发现不一致周末修正
...
| 7/13(一) ~ 7/17(四) | 四人完成核心检测逻辑，7/18(五) **接口对齐检查** |
| 7/18(五) ~ 7/21(一) | 单元测试 + 异常处理，周末修正接口问题 |
| 7/22(二) ~ 7/24(四) | 联调 + PR 合入 main |
| 7/25(五) ~ 7/26(六) | PPT + 演示数据准备 |
| 7/27(日) | **内部预演**，发现并修正问题 |
| 7/28(一) ~ 7/29(二) | 汇报前打磨 |
| **7/29(三)** | ★ **项目分享汇报** |
